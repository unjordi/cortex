...
- **Barrido de DOS eras viejas (`claude-quota` Y `claude-brain`):** hubo dos renames, `claude-quota → claude-brain → cortex`. Cada instalador (`macos/install.sh`, `install.sh`, `windows/install.ps1`) barre AMBAS eras + los 3 uninstallers también cubren `claude-brain`. macOS: ambos plists (`io.github.unjordi.claude-brain` daemon + `.widget`), `claude-brain-fetch`, `Claude Brain Widget.app`, cache/config. Linux: units `claude-brain.{timer,service}`, fetch, cache/config y plasmoid `io.github.unjordi.claude-brain`. Windows: proceso/Run key/`Programs\ClaudeBrain`/cache `%LOCALAPPDATA%\claude-brain`/`Claude Brain.lnk`. **SOLO se borra `claude-brain-fetch`; los helpers compartidos de `~/.local/bin` NO.** **GOTCHA #312:** el rename mecánico dejó los strings de limpieza apuntando a artefactos que nunca existieron (`old_marker` del PATH `# cortex: …claude-quota-fetch`; `OLD_DIR` de bootstrap `~/cortex` / `%USERPROFILE%\cortex`); el barrido `claude-brain` (rama `feat/instalador-limpia-era-claude-brain`) los corrigió a los strings/dirs que las eras previas SÍ escribieron.
- **Clon oculto del bootstrap (PR #138):** `bootstrap.sh` → `~/.cortex`; `bootstrap.ps1` → `%LOCALAPPDATA%\cortex-repo` (`-repo` para no chocar con el cache `%LOCALAPPDATA%\cortex` ni la app `%LOCALAPPDATA%\Programs\Cortex`). El autoupdate lo NECESITA (no se puede borrar tras instalar). `brain-scan.sh` conoce la ruta nueva. La migración del clon reubica el clon de la era `claude-brain` (`~/.claude-brain`/`~/claude-brain`; Windows `%LOCALAPPDATA%\claude-brain-repo`/`%USERPROFILE%\claude-brain`) → `$DIR`/`$dir` + reapunta el remote. **GUARD DURO:** salta la iteración si la ruta vieja resuelve al MISMO destino activo (`-ef`/string en bash, `Resolve-Path` en PS) — sin esto, con `CLAUDE_BRAIN_DIR` persistido en forward-slash por la era claude-brain, todo upgrader Windows entraba a la rama de borrado y `rm -rf`-eaba su clon VIVO.
