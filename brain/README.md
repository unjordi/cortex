...
| `contrato-hilo.sh` (lib) | — | **El CONTRATO del hilo, en UNA definición.** El footer `> Última actualización: <AAAA-MM-DD> · rama <rama> · nivel <x>` no es decoración: con él `rehidratar-hilo` decide vigencia. La lib expone `hilo_rama` / `hilo_fecha` / `hilo_edad_legible` / `verificar_hilo`; el hook la SOURCEA (con fallback inline si falta: fail-open), el skill `checkpoint` la corre al volcar (fail-loud) y `verificar-contrato-hilo.sh` la corre SOLA en cada escritura (ver abajo). Medido 2026-09-11: **2 de 9 hilos reales de `~/code` no traían el footer** ⇒ su hilo VIGENTE se degradaba a "OBSOLETO" en cada rehidratado, sin que nada lo detectara. |
| `verificar-contrato-hilo.sh` | PostToolUse/Edit\|Write\|MultiEdit | AVISA (no bloquea, fail-open) cuando se escribe `hilo-mental-actual.md` sin el footer que exige `contrato-hilo.sh`. Antes `verificar_hilo()` solo corría si el modelo la invocaba a mano desde la prosa de `checkpoint` (M3, auditoría 2026-09-15: 3 pasadas independientes confirmaron que nada más la disparaba); ahora corre SOLA tras cada `Edit`/`Write`/`MultiEdit` sobre ese archivo, mismo patrón que `proteger-fuente-cerebro.sh`. Escape: `CLAUDE_SKIP_VERIFICAR_HILO=1`. |
