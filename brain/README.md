...
| `barrer-ramas.sh` | SessionStart 🔔 Automático | Da TRIGGER al barrido de ramas locales ya integradas: al abrir sesión en un repo git, ≤1× por `BARRER_RAMAS_HORAS` (def 24), lanza `limpiar-ramas.sh` EN SEGUNDO PLANO para borrar las ramas zombie (MR squasheado → remota borrada → local `: gone`, o commits ya en la base por equivalencia de parche). CONSERVA todo trabajo sin integrar y NUNCA toca la actual/base/`develop`/`main`/`Develop*`/`keep/*`. Es el MECANISMO que dispara al `script` `limpiar-ramas` (que nadie más ejecutaba → las ramas squasheadas se acumulaban). |
...
| `recordar-cosechar.sh` | Stop | Nudge GENTIL y NO bloqueante de "trabajaste y no dejaste memoria durable", con UN solo aviso al día que cubre DOS señales: si hubo trabajo sustantivo reciente pero (1) `.claude/memory/aprendizajes.md` no se tocó → recuerda `/cosechar-sesion`, y/o (2) el backlog durable (`.claude/memory/estado-proyecto.md` / `bitacora.md`) no se tocó → recuerda reflejar lo avanzado/decidido/parqueado. El mensaje es condicional (menciona solo lo que falte). Mitad "recuérdame" del par con la skill `cosechar-sesion`; alimenta el inbox de aprendizajes y el backlog vivo del equipo. |
