...
| `recordar-cosechar.sh` | Stop | **ESPEJO automático e idempotente** — vuelca los PENDIENTES del TaskList vivo de la sesión a un bloque fenced `<!-- espejo-tasklist -->` dentro de `.claude/memory/estado-proyecto.md` (determinista, lee `~/.claude/tasks/<sid>/*.json` con jq → markdown; SIN LLM; solo si el .md ya existe, no lo crea; solo toca ese bloque). Nunca bloquea. (El NUDGE que traía antes —cosecha/backlog sin tocar— se retiró overhaul hooks 2026-09-18, puramente advisory: su regla subió a norma en `brain/norms/global-claude-md.md`.) |
