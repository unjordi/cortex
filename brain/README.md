...
| `recordar-cosechar.sh` | Stop | Nudge GENTIL y NO bloqueante de "trabajaste y no cosechaste": al terminar un turno, si en ESTE repo hubo trabajo sustantivo reciente pero `.claude/memory/aprendizajes.md` no se tocó, recuerda correr `/cosechar-sesion` antes de cerrar. Mitad "recuérdame" del par con la skill `cosechar-sesion`; alimenta el inbox de aprendizajes del equipo. |
