...
- REGLA DE NEGACIÓN EXPLÍCITA (DOMINA sobre la celebración y el verde técnico): si el mensaje contiene una NEGACIÓN EXPLÍCITA de cierre ('nada declarado LISTO', 'nada de esto está listo', 'no lo doy por listo', 'no está listo todavía') O CALIFICA el claim con un marcador de estatus ('verificado técnicamente, pendiente tu QA', 'en preview', 'con tu OK', 'esperando tu OK') → CIERRE=no, SIN IMPORTAR cuánta CELEBRACIÓN ('🎉', '🚀', 'hito', 'qué bonito quedó') ni cuántas afirmaciones de que algo 'existe / corre / funciona / quedó / hecho / resolvió end-to-end' lo rodeen. Declarar que algo funciona TÉCNICAMENTE, o celebrar el avance, NO es un claim de cierre cuando el asistente EXPLÍCITAMENTE dice que NO lo da por listo, o que espera el QA/OK del usuario. Esto NO afloja el eje: un claim de cierre REAL y SIN negación ni calificador de estatus ('el módulo quedó listo', 'ya funciona el widget') SIGUE siendo CIERRE=si.
